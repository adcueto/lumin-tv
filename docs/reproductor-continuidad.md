# Reproductor Roku — continuidad y recuperación (app 5.2 build 62)
...
`python3 roku-app/pruebas/correr.py` → planificación 42 comprobaciones y
escena 21 comprobaciones, 0 fallos (build 62). La escena cubre el comando
`recargar` (respuesta idéntica, distinta, sin red, sin lista). Eso demuestra
*qué* se intenta y *cuándo*; no demuestra la transferencia, la reproducción,
los observadores de SceneGraph ni `cachefs:`.
...
| F22 | Estado real en el panel (B5a) | Con la app **build 62** contra un servidor 6.10.1 de pruebas del **mismo SHA que la app** (`git log -1` del paquete; la versión 6.10.1 por sí sola no distingue el arreglo de permisos), abrir el panel | Bajo la pantalla aparece "▶ <archivo> · cache N MB · app 5.2.61 · <modelo> · OS x · <IP>"; el archivo coincide con lo que se ve en la TV y cambia al cambiar de elemento |
| F23a | Recargar con red (B5a) | Con la TV en línea y reproduciendo el 3.º elemento, pulsar ↻ **sin cambiar la lista** (respuesta idéntica); repetir tras cambiar un elemento (respuesta distinta). Anotar la hora del clic y mirar `telnet IP 8085` | En ambos casos, en su siguiente latido (≤ 4 s) la TV corta lo que estaba y arranca desde el primer elemento; nunca queda en negro; en el panel desaparece el "último error" si lo había. En el log: `recargar` seguido de la reconstrucción de segmentos |
| F23b | Recargar y perder la red antes del refresco | Pulsar ↻ y, **en cuanto el log muestre `recargar`**, cortar el WAN (el punto de corte es después de que el comando llegó y antes del refresco) | La TV sigue reproduciendo desde el primer elemento con lo que tiene en cache; no aparece la lámina si hay contenido; al reconectar, el latido reporta `k=1` mientras siga sin red y se normaliza después |
| F23c | Recargar emitido durante la desconexión | Cortar el WAN, pulsar ↻ en el panel (el comando queda en el servidor, la TV no lo recibe), reconectar | Coherente con F10 y la política de comandos antiguos: `recargar` **no** es `reproducir`, así que **sí se ejecuta** al reconectar (describe un estado deseado, como pausa). Anotar si se prefiere lo contrario: es decisión de producto, no defecto |
