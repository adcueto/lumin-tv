...
**Sin cambios.** La app sigue consumiendo `GET /playlist.json?id=…` con la
misma respuesta, incluidos `comando`, `turno` (con `ts` y `duracion`, que ya
existían) y las URLs de `/videos/…` y `/miniaturas/…`. El servidor 6.9 no
necesita tocarse para desplegar la app 5.2, y la app 5.1 sigue funcionando
contra el mismo servidor durante la convivencia.
